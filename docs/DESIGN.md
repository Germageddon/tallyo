...
- **Owner-only `/stats`.** Matched on `OWNER_ID` (the operator's `platform_user_id`); returns total users, users who logged an expense, active-in-7-days, and expense count. To any non-owner it returns the plain "Unknown command" reply — no data, no hint the command exists. Not in `setMyCommands`, so it never shows in the `/` menu.
...
- **Range picking is tap-only.** Besides the preset periods, the period menu offers **By month** (a Jan-to-Dec grid for one year, with year arrows; `months:<kind>[:<year>]` → `mon:<kind>:<YYYY-MM>`, also reachable as `/month`) and **Custom range**, an inline **calendar** (`src/app/calendar.ts`, Monday-first grid). The calendar is two taps: `cal:<kind>[:<YYYY-MM>][:<from>]` renders a month, `cpick:<kind>:<YYYY-MM-DD>[:<from>]` sets the start on the first tap and runs the range on the second, swapping the dates if they were tapped out of order. Padding cells use a `noop` action that returns no reply. **Picker screens edit in place**: a `buttons` reply may carry `edit: true`, which tells an adapter to replace the message the tap came from (Telegram: `ctx.editMessageText`, falling back to a fresh message on any error other than "message is not modified"). Calendar screens, the month grid, and `pick:<kind>` (Back) set it, so paging never floods the chat; **opening** a picker and every **result** (report, export) deliberately do not, because the Report button also lives on the "Logged" message and editing there would destroy it plus its Undo. Typing `/report 2026-01-01 2026-03-31` still works but is no longer the only way to get an arbitrary range.
